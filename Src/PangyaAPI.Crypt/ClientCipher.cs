...
namespace PangyaAPI.Crypt
{
    ...
    public static class ClientCipher
    {
        ...
        {
            if (key >= 32) throw new ArgumentOutOfRangeException(nameof(key), $"Key too large ({key} >= 0x10)");

            if (source.Length < 5)
                throw new ArgumentOutOfRangeException(nameof(source), $"Packet too small ({source.Length} < 5)");

            int ParseKey = key << 8;
            int tmpbyte = source[4];
            var buffer = (byte[]) source.Clone();

            buffer[4] = Convert.ToByte(CryptoOracle.CryptTable[source[0] + ParseKey] ^ tmpbyte);
