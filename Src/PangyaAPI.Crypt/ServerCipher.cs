...
namespace PangyaAPI.Crypt
{
    ...
    public static class ServerCipher
    {
        ...
        public static byte[] Decrypt(byte[] source, byte key)
        {
            ...
            byte tmpbyte2 = Convert.ToByte(CryptoOracle.CryptTable[source[0] + ParseKey] ^ tmpbyte);
        ...
        public static byte[] Encrypt(byte[] source, byte key, byte salt)
        {
            ...
            var tmpbyte = CryptoOracle.CryptTable[3840];
            var u = source.Length;
            var x = (u + u / 255) & 0xff;
            var v = (u - x) / 255;
            var y = (v + v / 255) & 0xff;
            ...
            buffer[7] ^= CryptoOracle.CryptTable[oracleIndex % 256];
            return buffer;
        }
    }
}