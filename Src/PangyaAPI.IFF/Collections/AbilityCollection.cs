        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var IffStructSize = Tools.IFFTools.SizeStruct(new Ability());
                    if (IffStructSize != recordLength)
                    {
                        throw new Exception($"Ability.iff the structure size is incorrect, Real: {recordLength}, Ability.cs: {IffStructSize} ");
                    }
            ...
            {
                MessageBox.Show("[Error Struct]:  " + ex.Message, "Pangya.IFF.Model.Ability");
                return false;
            }
        }
