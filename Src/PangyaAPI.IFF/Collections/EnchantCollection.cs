        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var IffStructSize = Tools.IFFTools.SizeStruct(new Enchant());
                    if (IffStructSize != recordLength)
                    {
                        throw new Exception($"Enchant.iff the structure size is incorrect, Real: {recordLength}, Enchant.cs: {IffStructSize} ");
                    }
            ...
            {
                MessageBox.Show("[Error Struct]:  " + ex.Message, "Pangya.IFF.Model.Enchant");
                return false;
            }
        }
