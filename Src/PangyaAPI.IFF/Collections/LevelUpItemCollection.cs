        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var IffStructSize = Tools.IFFTools.SizeStruct(new LevelUpPrizeItem());
                    if (IffStructSize != recordLength)
                    {
                        throw new Exception($"LevelUpPrizeItem.iff the structure size is incorrect, Real: {recordLength}, LevelUpPrizeItem.cs: {IffStructSize} ");
                    }
            ...
            {
                MessageBox.Show("[Error Struct]:  " + ex.Message, "Pangya.IFF.Model.LevelUpPrizeItem");
                return false;
            }
        }
