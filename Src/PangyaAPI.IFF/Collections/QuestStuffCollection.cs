        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var IffStructSize = Tools.IFFTools.SizeStruct(new QuestStuff());
                    if (IffStructSize != recordLength)
                    {
                        throw new Exception($"QuestStuff.iff the structure size is incorrect, Real: {recordLength}, QuestStuff.cs: {IffStructSize} ");
                    }
            ...
            {
                MessageBox.Show("[Error Struct]:  " + ex.Message, "Pangya.IFF.Model.QuestStuff");
                return false;
            }
        }
