        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var IffStructSize = Tools.IFFTools.SizeStruct(new Club());
                    if (IffStructSize != recordLength)
                    {
                        throw new Exception($"Club.iff the structure size is incorrect, Real: {recordLength}, Club.cs: {IffStructSize} ");
                    }
            ...
            {
                MessageBox.Show("[Error Struct]:  " + ex.Message, "Pangya.IFF.Model.Club");
                return false;
            }
        }
