        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var IffStructSize = Tools.IFFTools.SizeStruct(new QuestItem());
                    if (IffStructSize != recordLength)
                    {
                        throw new Exception($"QuestItem.iff the structure size is incorrect, Real: {recordLength}, QuestItem.cs: {IffStructSize} ");
                    }
            ...
            {
                MessageBox.Show("[Error Struct]:  " + ex.Message, "Pangya.IFF.Model.QuestItem");
                return false;
            }
        }
