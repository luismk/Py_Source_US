        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var IffStructSize = Tools.IFFTools.SizeStruct(new Match());
                    if (IffStructSize != recordLength)
                    {
                        throw new Exception($"Match.iff the structure size is incorrect, Real: {recordLength}, Match.cs: {IffStructSize} ");
                    }
            ...
            {
                MessageBox.Show("[Error Struct]:  " + ex.Message, "Pangya.IFF.Model.Match");
                return false;
            }
        }
