        ...
        public bool Load(MemoryStream data)
        {
            ...
            try
            {
                using (var Reader = new PangyaBinaryReader(data))
                {
                    ...
                    var IffStructSize = Tools.IFFTools.SizeStruct(new TikiPointTable());
                    if (IffStructSize != recordLength)
                    {
                        throw new Exception($"TikiPointTable.iff the structure size is incorrect, Real: {recordLength}, TikiPointTable.cs: {IffStructSize} ");
                    }
            ...
            {
                MessageBox.Show("[Error Struct]:  " + ex.Message, "Pangya.IFF.Model.TikiPointTable");
                return false;
            }
        }
