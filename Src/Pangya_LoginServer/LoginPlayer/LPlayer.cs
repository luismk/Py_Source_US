...
namespace Pangya_LoginServer.LoginPlayer
{
    public class LPlayer : Player
    {
        ...
        public void HandlePlayerLogin(Packet ClientPacket, bool OpenServer)
        {
            string Nickname, Auth1, Auth2;
            Byte Code, Banned, FirstSet;
            int UID;
            USP_LOGIN_SERVER_US_Result UspLoginServer = null;

            if (OpenServer == false)
            {
                HandleLoginMessage(LoginCodeFlag.ServerInMaintenance);
            ...
            try
            {
                ...
                UspLoginServer = _db.USP_LOGIN_SERVER_US(User, Pwd, GetAdress, Auth1, Auth2).FirstOrDefault();
                ...
                if (OUTCODE || Code == 5)
                {
                    HandleLoginMessage(LoginCodeFlag.InvalidoUsernameOuSenha);
                    Disconnect();
                    return;
                }
                ...
                if (Code == 6)
                {
                    HandleLoginMessage(LoginCodeFlag.InvalidoIdPw);
                ...
                if (Banned > 0)
                {
                    HandleLoginMessage(LoginCodeFlag.Banido);
                    Disconnect();
                    return;
                }
                ...
                {
                    HandleLoginMessage(LoginCodeFlag.UsuarioEmUso);
                    return;
                }
                ...
                if (string.IsNullOrEmpty(Nickname))
                {
                    ...
                    HandleLoginMessage(LoginCodeFlag.CreateNickName_US);
                    return;
                }
                ...
                if (FirstSet == 0)
                {
                    ...
                    HandleLoginMessage(LoginCodeFlag.CreateNickName);
                    return;
                }
            }
            catch
        ...
        private void SendPlayerLoggedOnData()
        {
            ...
            HandleLoginMessage(LoginCodeFlag.Sucess);
        ...
        public void HandleSendAuthKeyLogin()
        {
            Response.Clear();
            Response.WriteUInt16(0x0010);
            if (GetAuth1 == "")
            {
                GetAuth1 = RandomAuth(7);
            }
            Response.WritePStr(GetAuth1);//AuthKeyLogin
            Send(Response.GetBytes());
        }

        public void SendGameAuthKey()
        {
            Response.Clear();
            ...
            if (GetAuth2 == "")
            {
                GetAuth2 = RandomAuth(7);
            }
            Response.WritePStr(GetAuth2);
            Send(Response.GetBytes());
        }

       public void HandlePlayerReconnect(Packet packet)
        {
        ...
        public void CreateCharacter(Packet ClientPacket)
        {
        ...
        public void HandleDuplicateLogin()
        {
        ...
        public void NicknameCheck(Packet ClientPacket)
        {
            Byte Code;
            ...
            if ((Code == 0) || (Code == 2))
            {
                HandleMessageBoxNickName(ConfirmNickNameFlag.Indisponivel, "");
                return;
            }

            if (Code == 1)
            {
                HandleMessageBoxNickName(ConfirmNickNameFlag.Disponivel, Nickname);
            }
        }

        public void RequestCharacaterCreate(Packet ClientPacket)
        {
        ...
        void HandleLoginMessage(LoginCodeFlag msgType)
        {
            Response.Clear();
            Response.Write(new byte[] { 0x01, 0x00 });
            Response.WriteByte((byte)msgType);
            if (msgType != LoginCodeFlag.Sucess)
            {
        ...
        void HandleMessageBoxNickName(ConfirmNickNameFlag msgType, string nickname)
        {
            Response.Clear();
            Response.Write(new byte[] { 0x0E, 0x00 });
            Response.WriteUInt32((uint)msgType);
            //Nickname disponivel
            if (msgType == ConfirmNickNameFlag.Disponivel)
            {
