...
namespace Pangya_LoginServer.ServerTcp
{
    public class LoginServer : TcpServer
    {
        public LoginServer()
        {
            try
            {
                ...
                Console.Title = "Pangya Login Server - Fresh Up!";
            }
        ...
        public override void ServerStart()
        {
            try
            {
                ...
                // Data.InsertServer();
                Start();
                WriteConsole.WriteLine($"[LOGINSERVER]: START -> PORT {Data.Port}", ConsoleColor.Green);
        ...
        protected override Player OnConnectPlayer(TcpClient tcp)
        {
            ...
            WriteConsole.WriteLine($"[PLAYER_NEW_CONNECTION]: {player.GetAdress}:{player.GetPort} | KEY: {player.Key}", ConsoleColor.Green);
        ...
        {
            var US = new byte[] { 0x00, 0x0B, 0x00, 0x00, 0x00, 0x00, player.Key, 0x00, 0x00, 0x00, 0x0F, 0x27, 0x00, 0x00 };
