...
namespace PangyaAPI.Helper.BinaryModels
{
    public class PangyaBinaryReader : BinaryReader
    {
        ...
        public uint GetSize { get { return (uint)BaseStream.Length; } }

        public uint GetPosition { get { return (uint)BaseStream.Position; } }
        ...
        //não testado
        public bool Read(ref object value, int Count)
        {
            try
            {
                value = new object();
        ...
        public T ReadStruct<T>(int Count) where T : struct
        {
            ...
            local = (T)Marshal.PtrToStructure(ptr, local.GetType());
            Marshal.FreeHGlobal(ptr);
        ...
        public Object ReadObject(object obj, int Count = 0)
        {
            foreach (var property in obj.GetType().GetProperties())
            {
                Type type = property.PropertyType;
                ...
                if (Count > 0 && (Count + 2) == GetPosition)
                {
                    return obj;
                }
